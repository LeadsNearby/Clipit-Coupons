jQuery(document).ready(function(){	
	...
	jQuery('#coupon_type').change(function(){
	  ...
	  if(jQuery(this).val() == 'Build'){
		...
		jQuery("#coupon_action").find("option[value='promo']").show();		

		jQuery("#coupon_action").find("option[value='peel']").show(); 
		...
		jQuery("#coupon_action").find("option[value='promo']").hide();		

		jQuery("#coupon_action").find("option[value='peel']").hide();
	...
	jQuery('.upload_coupon_button').on( "click", function(e) {
	   ...
	   e.preventDefault();
        ...
        coupon_uploader.on('select', function() {
			...
			$coup.closest('p').find('.upload_image').val(attachment.url);		
	...
	if(jQuery('.taxonomy-locations').length > 0){		
		...
		jQuery('#tag-description').closest('.form-field').find('label').text('Enter any additional location information.');		

		jQuery('#tag-description').closest('.form-field').find('p').text('Enter any additional location information. You can use HTML.');		
		...
		jQuery('.form-field').find('#description').next().next('.description').text('Enter any additional location information. You can use HTML.');	
	...
	jQuery("#tabs-1 .reset").click(function () {		
		...
		jQuery('#coupon_theme .header-sample').click(function(e){		
			...
			jQuery('img.highlight').not(e.target).removeClass('highlight');		
		...
		jQuery("#coupon_theme input[type=radio][checked]").each(function() { 		
			...
			jQuery(this).next('label').find('.header-sample').addClass('highlight');	
